...
def render_property_carousel(
    state: Annotated[AppState, InjectedState],
    tool_call_id: Annotated[str, InjectedToolCallId],
) -> Command:
    """
    Renders a property carousel UI from properties in state and returns an AI message.
    ...
    try:
        ...
        push_ui_message("property_carousel", {"properties": properties_as_dicts}, message=ai_message)

        # Create success tool message
        success_message = f"Successfully created property carousel UI for {property_count} properties."

        return Command(
            update={"messages": [ToolMessage(content=success_message, tool_call_id=tool_call_id), ai_message]}
        )
